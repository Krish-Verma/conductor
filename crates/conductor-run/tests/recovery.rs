...
/// Seed the parent rows recovery needs.
///
/// `source` is the **registered tree**. It stopped being a fiction at S12: §6.5's
/// packet is generated from durable state plus the plan document in that tree, so a
/// project row pointing at `/fixture` means an attempt that cannot be told what to
/// do.
